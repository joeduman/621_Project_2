...
Each time an experiment is run, **.log** files are saved for the fileserver, main (dispatcher & consolidator), and each worker. When all workers are finished, the script combines the worker logs into one file for easier parsing (see above output to see what is stored in the log files). 
